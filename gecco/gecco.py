...
class Corrector:
    ...
    def parseconfig(self,configfile):
        ...
        config = yaml.full_load(open(configfile,'r',encoding='utf-8').read())

        if 'inherit' in config:
            baseconfig = yaml.full_load(open(config['inherit'],'r',encoding='utf-8').read())
    ...
    def main(self):
        ...
        if args.command == 'run':
            ...
            servers = self.findservers()
            if not servers:
                print("No servers are running (root=",self.root+")", file=sys.stderr)
            else:
                for module, host, port, load in servers:
